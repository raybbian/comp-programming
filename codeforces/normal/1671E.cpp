...
void solve() {
    ...
    vector<char> s(pow);
    rep(i, 1, pow - 1) { cin >> s[i]; }

    auto dfs = [&](int p, auto &&dfs) -> pair<mint, string> {
        if (p >= pow / 2) {
            return make_pair(1, to_string(s[p]));
        }
        const auto &[x, sx] = dfs(2 * p, dfs);
        const auto &[y, sy] = dfs(2 * p + 1, dfs);
        mint sol = 0;
        if (sx == sy) {
            sol = x * x;
        } else {
        ...
        string check = min(sy + s[p] + sx, sx + s[p] + sy);
        return make_pair(sol, check);
    };
