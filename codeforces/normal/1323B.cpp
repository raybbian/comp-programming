...
void solve() {
	...
	if (b[m] == 1) {
		b_seg.push_back(m - l + 1);
	}
	dbg(a_seg, b_seg);
	//maybe count such that all a is 1
	//then remove rectanges that touch that row
	ll ans = 0;
	for(ll i = 1; i * i <= k; i++) {
		if (k % i == 0) {
			//width is i, height is k/i
			ll w_seg = 0;
			for(auto &j: a_seg) {
				int cont = max((ll)0, j - i + 1);
				w_seg += cont;
			}
			ll h_seg = 0;
			for(auto &j: b_seg) {
				int cont = max((ll)0, j - (k/i) + 1);
				h_seg += cont;
			}
			ans += w_seg*h_seg;
			if (i == k/i) continue;
