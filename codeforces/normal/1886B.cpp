...
void solve() {
  ...
  // both covered by A
  // both covered by B
  // one covered by A, one covered by B, AB touching
  long double ans = LLONG_MAX;
  // both covered by A
  long double dist_O_A = sqrt(ax * ax + ay * ay);
  long double dist_P_A = sqrt((ax - px) * (ax - px) + (ay - py) * (ay - py));
  ans = min(ans, max(dist_O_A, dist_P_A));
  // both covered by B
  long double dist_O_B = sqrt(bx * bx + by * by);
  long double dist_P_B = sqrt((bx - px) * (bx - px) + (by - py) * (by - py));
  ans = min(ans, max(dist_O_B, dist_P_B));
  // O covered by A, P covered by B, AB touching
  long double min_w_touching =
      sqrt((ax - bx) * (ax - bx) + (ay - by) * (ay - by)) / 2;
  ans = min(ans, max({min_w_touching, dist_O_A, dist_P_B}));
  ans = min(ans, max({min_w_touching, dist_P_A, dist_O_B}));
  cout << setprecision(11) << ans << ln;
}
