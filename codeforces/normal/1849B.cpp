...
void solve() {
	int n, k;
	cin >> n >> k;
	set<pii, decltype(cmp)> health(cmp);
	for(int i = 0; i < n; i++) {
		int a;
		cin >> a;
		if (a % k == 0) a = k;
	...
	dbg(health);
	while(!health.empty()) {
		if (k >= prev(health.end())->fi) {
			cout << prev(health.end())->se << " ";
			health.erase(prev(health.end()));
		} else {
			int a = prev(health.end())->fi - k;
			int i = prev(health.end())->se;
			health.erase(prev(health.end()));
			health.insert({a, i});
		}
	}
