...
void solve() {
	...
	//a % (c / b) == 0
	//(a * b) % c == 0
	//largest c such that i - 1 and i share name tag is gcd
	//c % b == 0
	//l to r can use nametag
	//if gcd(a*b, a*b, a*b...) % lcm(b, b, b) == 0
	ll ans = n;
	ll l = 0, r = 1;
	ll gcd1 = candies[l].fi * candies[l].se;
	ll lcm1 = candies[l].se;
	while(l < r && r < n) {
		ll a = candies[r].fi;
		ll b = candies[r].se;
		gcd1 = gcd(gcd1, a * b);
		...
		if (gcd1 % lcm1 == 0) {
			...
			l = r;
			gcd1 = candies[l].fi * candies[l].se;
			lcm1 = candies[l].se;
		}
		r++;
	}
