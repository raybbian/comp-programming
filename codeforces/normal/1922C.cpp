...
void solve() {
  ...
  vector<bool> closest(n + 1); // goes right
  closest[1] = true;
  closest[n] = false;
  for (int i = 2; i < n; i++) {
    closest[i] = (cities[i] - cities[i - 1] > cities[i + 1] - cities[i]);
  }

  dbg(cities, closest);
  vector<int> pref_right(n + 1);
  pref_right[1] = 0;
  for (int i = 2; i <= n; i++) {
    pref_right[i] =
        pref_right[i - 1] + (closest[i - 1] ? 1 : cities[i] - cities[i - 1]);
  }

  vector<int> pref_left(n + 2);
  pref_left[n] = 0;
  for (int i = n - 1; i >= 1; i--) {
    pref_left[i] =
        pref_left[i + 1] + (!closest[i + 1] ? 1 : cities[i + 1] - cities[i]);
  }
  dbg(pref_right, pref_left);
  ...
  for (int i = 0; i < m; i++) {
    ...
    cin >> x >> y;
    if (x < y) {
      cout << pref_right[y] - pref_right[x] << "\n";
    } else {
      cout << pref_left[y] - pref_left[x] << "\n";
    }
  }
}
