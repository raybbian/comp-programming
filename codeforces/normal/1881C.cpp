...
void solve() {
  int n;
  cin >> n;
  vector<vector<char>> matrix(n, vector<char>(n, '_'));
  for (int i = 0; i < n; i++) {
    string row;
    cin >> row;
    for (int j = 0; j < n; j++) {
  ...
  vector<vector<vector<char>>> freq(
      n, vector<vector<char>>(n, vector<char>(0, ' ')));
  vector<vector<char>> nmatrix(matrix);
  for (int cnt = 0; cnt < 4; cnt++) {
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < i; j++) {
        swap(nmatrix[i][j], nmatrix[j][i]);
      }
    }

    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n / 2; j++) {
        swap(nmatrix[i][j], nmatrix[i][n - j - 1]);
      }
    }

    for (int i = 0; i < n / 2; i++) {
      for (int j = 0; j < n / 2; j++) {
        freq[i][j].push_back(nmatrix[i][j]);
      }
    }
    dbg(nmatrix);
  }
  ...
  for (int i = 0; i < n / 2; i++) {
    for (int j = 0; j < n / 2; j++) {
      char best = *max_element(freq[i][j].begin(), freq[i][j].end());
      for (auto &s : freq[i][j]) {
        ans += best - s;
      }
    }
