...
void solve() {
	...
	cin >> s;
	s = " " + s;
	auto pref = ndvec<int>(26, n + 1, 0);
	for(int i = 1; i <= n; i++) {
		for(char col = 'a'; col <= 'z'; col++) {
			pref[col - 'a'][i] = pref[col - 'a'][i - 1] + (s[i] == col);
		}
	}
	dbg(pref);

	//for each color and max tiles, precompute koyomi value
	//can bruteforce
	auto ans = ndvec<int>(26, n + 1, 0);
	for(int col = 'a'; col <= 'z'; col++) {
		for(int i = 1; i <= n; i++) {
			for(int j = i; j <= n; j++) {
				//paint interval from i -> j
				int sz = j - i + 1;
				int colored = sz - (pref[col - 'a'][j] - pref[col - 'a'][i - 1]);
				chk_max(ans[col - 'a'][colored], sz);
			}
		}
	}
	for(int col = 'a'; col <= 'z'; col++) {
		for(int i = 1; i <= n; i++) {
			chk_max(ans[col - 'a'][i], ans[col - 'a'][i - 1]);
		}
	}
