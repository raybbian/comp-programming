...
void solve() {
	...
	cin >> n >> k;
	vector<int> jobs(n + 1);
	rep(i, 1, n) cin >> jobs[i];
	vector<ll> hot(k + 1), cold(k + 1);
	rep(i, 1, k) cin >> cold[i];
	rep(i, 1, k) cin >> hot[i];

	vector<ll> dp(k + 1, (ll)1e18); //dp[i] is program i ran on one cpu
	dp[0] = 0;
	rep(i, 1, n) {
		int x = jobs[i];
		int y = jobs[i - 1];
		vector<ll> ndp(k + 1, (ll)1e18);
		if (x == y)  {
			rep(j, 0, k) {
				if (j != x) ndp[x] = min(ndp[x], dp[j] + cold[x]);
				ndp[j] = min(ndp[j], dp[j] + hot[x]);
			}
			ndp[x] = min(ndp[x], dp[x] + hot[x]);
		}
		else  {
			rep(j, 0, k) {
				if (j != x) ndp[y] = min(ndp[y], dp[j] + cold[x]);
				ndp[j] = min(ndp[j], dp[j] + cold[x]);
			}
	...
	cout << *min_element(dp.begin(), dp.end()) << ln;
}
