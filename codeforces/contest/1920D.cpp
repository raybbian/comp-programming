...
void solve() {
  int n, q;
  cin >> n >> q;
  unsigned __int128 len = 0;
  vector<unsigned __int128> lens;
  ...
  for (int i = 0; i < q; i++) {
    ...
    cin >> _k;
    _k--;
    auto solve = [&](ll k, auto &&solve) -> void {
      auto it = upper_bound(lens.begin(), lens.end(), k);
      ll prev_len = *prev(it);
      int q_intr = it - lens.begin();
      // dbg(k, prev_len, q_intr);

      if (entr[q_intr].fi == 1) {
        // this was an add type, return val
        cout << entr[q_intr].se << " ";
      } else {
        solve(k % prev_len, solve);
      }
    };
    solve(_k, solve);
  }
