...
void solve() {
	int n;
	cin >> n;
	auto a = ndvec<int>(n, 0);
	for(int i = 0; i < n; i++) {
		cin >> a[i];
	}
	auto testFrom = ndvec<int>(n, 0);
	auto maxSuff = ndvec<int>(n + 1, 0);
	for(int i = n - 1; i >= 0; i--) {
		//if current suff is test, replace new num with 1 to get a multitest
		//if current suff is a multitest, 
		int dist = n - i - 1;
		if (i + a[i] + 1 < n && testFrom[i + a[i] + 1] != 0) {
			testFrom[i] = testFrom[i + a[i] + 1] + 1;
		} else if (a[i] == dist) {
			testFrom[i] = 1;
		}
		maxSuff[i] = max(testFrom[i], maxSuff[i + 1]);
	}
	auto num2 = ndvec<int>(n, 0);
	for(int i = n - 1; i >= 0; i--) {
		num2[i] = maxSuff[i + 1] + 1;
		if (i + a[i] + 1 < n) {
			num2[i] = max(num2[i], num2[i + a[i] + 1] + 1);
		}
	}
	for(int i = 0; i < n - 1; i++) {
		if (testFrom[i + 1] == a[i]) cout << 0 << " ";
		else if (testFrom[i + 1] != 0) cout << 1 << " ";
		else if (a[i] <= num2[i + 1]) cout << 1 << " ";
		else cout << 2 << " ";
	}
	cout << end1;
}
