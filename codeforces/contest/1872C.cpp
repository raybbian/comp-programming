...
void solve() {
	...
	cin >> l >> r;
	//find a + b in [l, r]
	//_gcd(a, b) cannot be 1
	//if [l,r] contains even number greater than 2 -> OK
	//if [l,r] contains only odd number -> two nums share divisor -> check if odd number is prime
	if (l == r && l % 2 == 1) {
		//check factors
		for(int i = 2; i * i <= l; i++) {
			if (l % i == 0) {
				//if l / i has remainder 0
				int other = l / i;
				cout << (i - 1) * other << " " << other << ln;
				return;
			}
		}
		cout << -1 << ln;
