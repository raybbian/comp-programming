...
void solve() {
	...
	//maximize x indices
	//minimize y indices
	//what happens if they share an index? disregard them
	//find how many indices individually someone has
	//then its n + n-1 + n-2 ... minus 1 + 2 + ...
	ll lowcm = lcm(x, y);
	//how many times does lowcm occur in n?
	ll occur_x = f_div(n, x);
	ll occur_y = f_div(n, y);
	ll occur_share = f_div(n, lowcm);
	occur_x -= occur_share;
	occur_y -= occur_share;
	//get the sum of the largest occur_x numbers, and subtract the sum of the least occur_y numbers
	ll sum_y = occur_y * (occur_y + 1) / 2;
	ll sum_x = (n - occur_x) * occur_x + occur_x * (occur_x + 1) / 2;
	cout << sum_x - sum_y << ln;
}
