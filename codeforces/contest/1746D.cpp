...
inline pll solve(int node, ll use, vector<vector<int>> &adj, vector<ll> &s) {
    int num_child = adj[node].size();
    pll res = {use * s[node], (use + 1) * s[node]};
    if (num_child == 0)
        return res;
    vector<ll> options;

    each(next, adj[node]) {
        const auto [lo_val, hi_val] = solve(next, use / num_child, adj, s);
        options.push_back(hi_val - lo_val);
        res.fi += hi_val;
        res.se += hi_val;
    }
    ...
    ll sum = 0;
    int t = use % num_child;
    rep(i, num_child - t - 1) sum += options[i];
    res.se -= sum;
    sum = 0;
    rep(i, num_child - t) sum += options[i];
    res.fi -= sum;
    return res;
}
