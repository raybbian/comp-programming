...
void solve() {
	...
	cin >> n >> k;
	//for i = 0; i <= n; i++
	//replace a_i with the smallest non-negative integer which does not occur in the collection
	vector<int> nums(n);
	...
	//repeats at k == 1, k == n + 1
	//smallest in array is a cycle
	//if missing 5, then a_1 will be 5, and a_1 will be missing
	//then a_2 = a_1, and a_2 will be missing
	//then a_3 = a_2, and a_3 will be missing
	//then a_n = a_n - 1, and a_n  will be missing
	//then repeat the cycle
	//
	//after one repetition, everything is shifted to the right, nums[0] = missing number
	//after two repetitions, everything is shifted to the right again, nums[0] = nums[n];
	//after three repetitions, everything is shifted to the right again, nums[0] = mmissing number;
	//cycles every k = c * n + 1 repetitions
	dbg(n, k);
	nums.push_back(firstNum);
	n++;
	rotate(nums.begin(), nums.begin() + ((n - k % n) + n) % n, nums.end());
	dbg(nums);
	for(int i = 0; i < n - 1; i++) {
		cout << nums[i] << " \n"[i == n - 2];
	}
