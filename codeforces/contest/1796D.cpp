...
void solve() {
    ...
    auto dp = ndvec<array<ll, 3>>(n + 1, k + 1, array<ll, 3>{(ll)-1e18, (ll)-1e18, (ll)-1e18});
    dp[0][0] = {0, 0, 0};
    rep(i, 1, n) {
        dp[i][0][0] = max({dp[i - 1][0][0]});
        dp[i][0][1] = max({dp[i - 1][0][0] + a[i] - x, dp[i - 1][0][1] + a[i] - x});
    ...
    rep(i, 1, n) {
        rep(j, 1, k) {
            ...
            dp[i][j][1] = max({dp[i - 1][j][0] + a[i] - x, dp[i - 1][j][1] + a[i] - x,
                               dp[i - 1][j - 1][0] + a[i] + x, dp[i - 1][j - 1][1] + a[i] + x});
            dp[i][j][2] =
                max({dp[i - 1][j][1], dp[i - 1][j - 1][1], dp[i - 1][j][2], dp[i - 1][j - 1][2]});
        }
    }

    dbg(dp);
    cout << max({dp[n][k][0], dp[n][k][1], dp[n][k][2], 0LL}) << "\n";
}
