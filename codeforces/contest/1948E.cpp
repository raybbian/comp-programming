...
vector<int> clique(int k) {
    vector<int> res;
    int m = c_div(k, 2);
    per(i, m, 1) { res.push_back(i); }
    per(i, k, m + 1) { res.push_back(i); }
    return res;
}
...
void solve() {
    int n, k;
    cin >> n >> k;
    vector<int> c = clique(k);
    int cnt = 0;
    rep(i, n / k) {
        rep(j, k) { cout << c[j] + i * k << " "; }
        cnt++;
    }
    vector<int> l = clique(n % k);
    rep(i, n % k) cout << l[i] + n / k * k << " ";
    cout << ln;
    cout << cnt + (n % k != 0) << ln;
    rep(i, n) { cout << i / k + 1 << " "; }
    cout << ln;
