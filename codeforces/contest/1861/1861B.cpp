...
void solve() {
	...
	cin >> num1 >> num2;
	int n = num1.length();
	//cant change trailing 1's to 0,
	//cant change leading 0's to 1
	//starting at one string end 1 -> find next 1 such that num2[index - 1] = 0
	for(int i = n - 1; i > 0; i--) {
		if (num1[i] == '1' && num2[i] == '1' && num1[i - 1] == '0' && num2[i - 1] == '0') {
			cout << "YES" << "\n";
			return;
		}
	}
