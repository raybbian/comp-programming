...
void solve() {
  ...
  // check if negative sum of top 1..k numbers is greater or less than 2 *
  // negative sum of next x numbers
  vector<int> pref(n);
  pref[0] = a[0];
  for (int i = 1; i < n; i++) {
    pref[i] = a[i] + pref[i - 1];
  }
  int min_rem = 2 * pref[x - 1];
  for (int i = 0; i < k; i++) {
    int a_rem_sum = pref[i];
    int b_rem_sum = 2 * (pref[min(n - 1, i + x)] - pref[i]);
    dbg(i, a_rem_sum, b_rem_sum);
    min_rem = min(min_rem, a_rem_sum + b_rem_sum);
  }
  cout << pref[n - 1] - min_rem << "\n";
}
