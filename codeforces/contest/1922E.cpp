...
void solve() {
  ...
  // to multiply number of subsets by 2, add increasing number to the end
  // to add number of subsets by 1, add decreasing number to end? -> all subsets
  // without it are same, must include it for it to work
  vector<int> actions;
  while (n > 2) {
    if (n % 2 == 0 && n / 2 > 2) {
      actions.push_back(2);
