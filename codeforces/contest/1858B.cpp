...
void solve() {
	int n, m, d;
	cin >> n >> m >> d;
	vector<int> cookie(m + 2);
	cookie[0] = 1;
	for(int i = 1; i <= m; i++) {
		cin >> cookie[i];
	}
	cookie[m + 1] = n + 1;
	//will eat cookie on first
	//if i has cookie seller, then eat
	//if been long time, eat
	//if cookie seller is between two d distance apart, can remove
	int cookie_cnt = m + 1;
	if (cookie[0] == cookie[1]) cookie_cnt--;
	for(int i = 1; i <= m + 1; i++) {
		if (cookie[i] - cookie[i - 1] > d) {
			cookie_cnt += f_div(cookie[i] - cookie[i - 1] - 1, d);
		}
	}
	dbg(cookie, cookie_cnt);

	map<int, int> ans;
	for(int i = 1; i <= m; i++) {
		int b_to_a = f_div(cookie[i + 1] - cookie[i - 1] - 1, d);
		int b_to_i = f_div(cookie[i] - cookie[i - 1] - 1, d);
		int i_to_a = f_div(cookie[i + 1] - cookie[i] - 1, d);
		dbg(b_to_a, b_to_i, i_to_a);
		if (b_to_a < b_to_i + i_to_a + 1) {
			ans[b_to_i + i_to_a + 1 - b_to_a]++;
		}
	}
	dbg(ans);
	if (ans.empty()) {
		cout << cookie_cnt << " " << m << ln;
		return;
	}
	cout << cookie_cnt - prev(ans.end())->fi << " " << prev(ans.end())->se << ln;
}
