...
void solve() {
    ...
    rep(i, m) {
        ...
        k--;
        a[k] -= d;
        auto p = trains.lower_bound({k, INT_MIN});
        if (p == trains.end() && trains.rbegin()->se > a[k]) {
            trains.insert({k, a[k]});
        } else if (p != trains.end() && p->fi == k) {
            trains.erase(p);
            trains.insert({k, a[k]});
        } else if (p != trains.begin() && prev(p)->se > a[k]) {
            trains.insert({k, a[k]});
        }

        while (true) {
            auto it = trains.lower_bound({k + 1, INT_MIN});
            if (it == trains.end() || it->se < a[k])
                break;
            trains.erase(it);
        }
        cout << trains.size() << " ";
