...
void solve() {
    int n, m;
    cin >> n >> m;
    vector<bool> bonus(n + 1), token(n + 1), good(n + 1);
    vector<vector<int>> adj(n + 1);
    vector<int> st, v(n + 1, 1e9);
    int p, b;
    cin >> p >> b;
    rep(i, p) {
        int a;
    ...
    set<int> good_tokens, bad_tokens;
    each(i, st) {
        each(j, adj[i]) {
            if (good[j])
                good_tokens.insert(i);
            else if (bonus[j])
                bad_tokens.insert(i);
        }
    }
    ...
    rep(i, 1, n) {
        if (!token[i] || v[i] > n)
            continue;
        has_ans |=
            (!good_tokens.empty() && (*good_tokens.begin() != i || *good_tokens.rbegin() != i));
        int cnt = bad_tokens.size();
        if (bad_tokens.count(i))
            cnt--;
        has_ans |= v[i] <= 1 + cnt;
    }
    cout << (has_ans ? "YES\n" : "NO\n");
