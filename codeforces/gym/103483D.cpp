...
void solve() {
  ...
  // binary search on dissatisfaction
  auto ok = [&](long long max_diss) -> bool {
    dbg(max_diss);
    vector<long long> n_b(b.begin(), b.end());

    int left = 1;
    for (int i = 1; i <= n; i++) {
      if (i - left > max_diss)
        left = i - max_diss;
      long long students = a[i];
      while (students) {
        while (left <= n && n_b[left] == 0)
          left++;
        if (left == n + 1 || left - i > max_diss)
          return false;
        long long put_amt = min(students, n_b[left]);
        students -= put_amt;
        n_b[left] -= put_amt;
      }
    }
  ...
  long long ans = first_true((ll)0, n - 1, ok);
  if (ans == n) {
    cout << -1 << ln;
  } else {
    cout << ans << ln;
