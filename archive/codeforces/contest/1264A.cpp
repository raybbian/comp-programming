...
void solve() {
	...
	p[0] = p[1];
	vector<int> switch_ind;
	for(int i = 1; i <= n; i++) {
		if (p[i] != p[i - 1]) {
			//can switch medals here
			if (i - 1 > n / 2) break;
			switch_ind.pb(i - 1);
		}
	}
	dbg(switch_ind);
	if (switch_ind.size() < 3) {
		cout << "0 0 0" << ln;
		return;
	}
	int l_ptr = switch_ind.back();
	int l = 0, r = 1;
	string ans = "0 0 0";
	int mx_size = 0;
	while(l < r && r < switch_ind.size()) {
		int num_gold = switch_ind[l];
		int num_silver = switch_ind[r] - switch_ind[l];
		int num_bronze = l_ptr - switch_ind[r];
		if (num_gold >= num_bronze) {
			dbg("FIN");
			break;
		}
		if (num_gold < num_silver && num_gold < num_bronze) {
			dbg(num_gold, num_silver, num_bronze);
			if (chk_max(mx_size, num_gold + num_silver + num_bronze)) {
				ans = to_string(num_gold) + " " + to_string(num_silver) + " " + to_string(num_bronze);
			}
		}
