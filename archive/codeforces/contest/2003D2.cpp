...
void solve() {
    ...
    rep(i, n) {
        ...
        max_mex = max(max_mex, mex[i]);
    }

    vector<int> cnt(maxl + 2, 0);
    vector<vector<int>> adj(maxl + 2, vector<int>());
    rep(i, n) {
        cnt[mex[i]]++;
        adj[mex[i]].push_back(next_mex[i]);
    }
 
    vector<int> dp(maxl + 2, -1);
    auto dfs = [&](int node, auto&& dfs) -> int {
        if (dp[node] != -1) return dp[node];
        int best = node;
        each(next, adj[node]) {
            best = max(dfs(next, dfs), best);
        }
        dp[node] = best;
        if (cnt[node] > 1) max_mex = max(max_mex, dp[node]);
        return best;
    };
    rep(i, 0, maxl + 1) {
        if (dp[i] == -1)
    ...
    rep(i, 0, checked) {
        ans += max(dp[i], max_mex);
    }
    long long rest = m * (m + 1) / 2 - (checked) * (checked + 1) / 2;
    if (checked == m) rest = 0;

    dbg(mex, next_mex, adj, dp, ans, checked, rest);
