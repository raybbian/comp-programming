...
void solve() {
    ...
    rep(i, 1, n) {
        if (i - 2 >= 1) {
            dp[i][0] = max(dp[i][0], dp[i - 3][0] + (mp[0][i] + mp[0][i - 1] + mp[0][i - 2] >= 2) +
                                         (mp[1][i] + mp[1][i - 1] + mp[1][i - 2] >= 2));
        }
        if (i - 3 >= 1) {
            dp[i][1] = max(dp[i][1], dp[i - 3][1] + (mp[0][i] + mp[0][i - 1] + mp[0][i - 2] >= 2) +
                                         (mp[1][i - 1] + mp[1][i - 2] + mp[1][i - 3] >= 2));
            dp[i][2] = max(dp[i][2], dp[i - 3][2] + (mp[1][i] + mp[1][i - 1] + mp[1][i - 2] >= 2) +
                                         (mp[0][i - 1] + mp[0][i - 2] + mp[0][i - 3] >= 2));
        }
        if (i - 1 >= 1) {
            dp[i][0] = max(dp[i][0], dp[i - 1][1] + (mp[0][i] + mp[1][i - 1] + mp[1][i] >= 2));
            dp[i][0] = max(dp[i][0], dp[i - 1][2] + (mp[0][i] + mp[0][i - 1] + mp[1][i] >= 2));

            dp[i][1] = max(dp[i][1], dp[i - 2][0] + (mp[0][i] + mp[0][i - 1] + mp[1][i - 1] >= 2));
            dp[i][2] = max(dp[i][2], dp[i - 2][0] + (mp[1][i] + mp[1][i - 1] + mp[0][i - 1] >= 2));
        }
    }
    dbg(dp);
    cout << dp[n][0] << '\n';
