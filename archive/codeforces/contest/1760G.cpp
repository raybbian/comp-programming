...
void solve() {
    ...
    unordered_set<int, chash> xor_a, xor_b;
    auto dfs = [&](int node, int x, int par, bool is_a, auto&& dfs) -> void {
        if (is_a && node == b && x != 0) {
            return;
        }
        if (is_a) xor_a.ins(x);
        else if (!is_a && node != b) xor_b.ins(x);

        for(auto &i: adj[node]) {
            if (i.fi == par) continue;
            dfs(i.fi, x ^ i.se, node, is_a, dfs);
        }
    };
    dfs(a, 0, -1, true, dfs);
    dfs(b, 0, -1, false, dfs);
