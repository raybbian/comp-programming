...
void solve() {
	...
	vector<mint> ev(k+1);
	//EV of first iteration is just average f
	//EV of second iteration is empty/ncmb2 * average f + m/ncmb2 * average f + 1/nmb2
	ev[1] = sumf / ncmb2;
	mint res = ev[1];
	rep(i, 2, k) {
		ev[i] = empty / ncmb2 * ev[i - 1] + m / ncmb2 * (ev[i - 1] + 1 / ncmb2);
		res += ev[i];
	}
	dbg(ev);
	cout << res << ln;
