...
void solve() {
  int n;
  cin >> n;
  vector<int> a(n + 1), suff_min(n + 2, INT_MAX), suff_max(n + 2, 0),
      pref_min(n + 2, INT_MAX), pref_max(n + 2, 0);
  for (int i = 1; i <= n; i++) {
    cin >> a[i];
  ...
  cout << (long double)1 << " ";
  for (int k = 2; k <= n; k++) {
    // choose t that makes the min of suff same as min of pref
    // or t that makes the max of suff same as max of pref
    ...
    max_r = suff_max[k];
    long double t1, t2;
    dbg(min_l, max_l, min_r, max_r);
    t1 = 1 - (long double)max_l / max_r;
    t2 = 1 - (long double)min_l / min_r;
    dbg(t1, t2);
    cout << max({t1, t2, (long double)0}) << " ";
  }
  cout << ln;
}
